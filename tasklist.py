...
#x.field_names = ["Change Number","State","Change Task Number","Short Description","Start Date","End Date","Assigned to"]
...
for record in response.all():
  ...
  ctask_number = record['number']
  if record['assigned_to'] == '':
      task_assinged_to = "No one"
  else:
      task_assinged_to = user.get(query={'sys_id': record['assigned_to']['value']}).one()['name']
  ...
  if change_state == 'Scheduled':
    x.add_row([change_number,change_state,ctask_number,change_start_date,change_end_date,task_assinged_to])
    #x.add_row([change_number,change_state,ctask_number,change_short_desc,change_start_date,change_end_date,task_assinged_to])
